...
class LeadsItem extends Component {
    ...
    render() {
                    ...
                    <Panel key={key}>
                        <Row className="show-grid">
                            <Col xs={12} md={10} sm={9}>
                                ...
                                <Col xs={8} md={10} sm={9}>
                                    <h3 className="green-text" onClick={() => browserHistory.push('/dashboard/leads/' + key)}>{jobObj.providerName}</h3>
                                    <h5>{jobObj.providerEmail} | www.wfh.io | {jobObj.timestamp}</h5>
                                    <p className='des'> {jobObj.desc} <a href="" className="green-text">read more</a>
                                    </p>
                                </Col>
                            </Col>
                            <Col xs={12} sm={3} md={2} className="action-btn-group">
                                <div className='btn btns green-bg' onClick={() => browserHistory.push('/dashboard/leads/' + key)}>View lead</div>
                                <div className='btn btns' style={{ backgroundColor: 'gray' }} onClick={() => _self.setState({ addtoListModal: true })}>Add to List</div>
        ...
        return (
            <div>
                 <AddtoListModel show={this.state.addtoListModal} onHide={closeAddtoListModal} />
               {jobCards}
            </div>
        );
    }
